...
- **2026-05-31 GPT-5.5 repair canary:** Ran the 12-word confirmed semantic repair seed with `gpt-5.5`, `reasoning_effort=none`, and 20k completion-token budget. It completed successfully and produced materially richer entries: assistant 2 meanings, doctor 4, charge 13, stock 9, post 11, course 8, bank 7, line 12, seat 6, mail 5, program 8, brush 7. The first GPT-5.5 attempt showed GPT-5-family models require `max_completion_tokens` instead of `max_tokens`, so live OpenAI generation, batch submission, and the canary script were updated for GPT-5-family request parameters.
- **2026-05-31 first semantic repair load:** Added `load_semantic_repairs_azure.ps1` and applied the 12 reviewed GPT-5.5 repair rows to Azure. The script backs up old rows to `semantic_repair_backup` before replacement; backup run id `dfdd1300-861c-4b01-a1a1-883605370b15`. Verification audit remained clean: 184,072 total rows, 0 missing stage, 0 word mismatches, 0 empty meanings, 0 missing Nastaliq. Azure now has 12 `gpt-5.5` definitions. Semantic-risk common assistant-like count dropped from 5,134 to 5,127; the remaining 5 repaired rows flagged by the heuristic are valid loanword primaries such as doctor/bank/course/program/brush.
- **2026-05-31 semantic repair scale-up:** Added OpenAI Batch scripts for semantic repair generation/collection. Exported the full remaining high-risk queue (5,122 rows before the first batch), submitted/collected/applied the first 100-row GPT-5.5 repair batch, and backed up old Azure rows under run id `1d8657c6-a03e-4f01-bb23-a477a026e706`. Production audit stayed clean after the load; Azure now has 112 `gpt-5.5` definitions and the common assistant-like risk count dropped to 5,066. Submitted the next 100-row GPT-5.5 repair batch `batch_6a1cd03c180c81909fdde9ce23dcaf68`; status was `validating` at last check.
- **2026-06-01 semantic repair batch 002:** Collected OpenAI batch `batch_6a1cd03c180c81909fdde9ce23dcaf68`: 100 accepted, 0 errors. Mechanical QA passed with only intentional Latin tokens in quoted/keyboard-shortcut examples. Applied all 100 repairs to Azure with backup run id `1e02c704-e2ff-4aeb-a8b1-d4c44f5c5650`. Production audit stayed clean; Azure now has 212 `gpt-5.5` definitions and common assistant-like risk dropped to 4,998. Exported remaining queue (4,922 rows) and submitted batch 003 `batch_6a1cd3b1f01881908012f29a8d5188af`, currently validating.
- **2026-06-01 semantic repair cost pivot:** User reported OpenAI balance dropping from about $96 to $76, so GPT-5.5 bulk repair was paused after already-paid batch 003. Batch 003 was collected and loaded: 100 accepted, 0 errors, backup run id `003fada8-9e2f-43a2-972f-6c0aeb41d1a9`. Production audit stayed clean; Azure now has 312 `gpt-5.5` definitions and common assistant-like risk dropped to 4,918. Official OpenAI pricing check confirmed GPT-5.4 mini is much cheaper ($0.75/$4.50 per 1M input/output; Batch API 50% off), so batch 004 was submitted with `gpt-5.4-mini` as `batch_6a1ce169860481908d978b9f974f9ba0` for the next 100 candidates; status was `validating`.
- **2026-06-01 GPT-5.4 mini repair trial:** Collected batch `batch_6a1ce169860481908d978b9f974f9ba0`: 100 accepted, 0 collection errors. Mechanical QA showed 5 minor flags; inspected flags were either loader-normalized display mismatches or intentional quoted Latin tokens. Applied the 100 repairs to Azure with backup run id `b2691d54-2c72-4401-9030-ff474d9299ed`. Production structural audit stayed clean and Azure now has 100 `gpt-5.4-mini` definitions. Semantic-risk common assistant-like count dropped to 4,829. However, spot review found enough questionable learner-quality choices (for example overly literal or less natural primaries in some rows) that further `gpt-5.4-mini` auto-submission is paused until a stronger semantic judge/review gate is added.
